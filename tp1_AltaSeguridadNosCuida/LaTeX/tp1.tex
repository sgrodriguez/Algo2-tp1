...
\tadOperacion{agregarE}{uni \ u, id \ i, est \ e, pos \ p}{uni}{$\lnot$ (def?(i, hippies?(u))) $\land$ $\lnot$ (def?(i, estudiantes?(u))) $\land$ posValida(p, u)}
\tadOperacion{agregarH}{uni \ u, id \ i, pos \ p}{uni}{$\lnot$ (def?(i, hippies?(u))) $\land$ $\lnot$ (def?(i, estudiantes?(u))) $\land$ posValida(p, u)}
...
\tadOperacion{moverHoE}{uni \ u, id \ i}{uni}{(def?(i, hippies?(u))) $\lor$ (def?(i, estudiantes?(u)))}
...
\tadOtrasOperaciones
 ...
 \tadOperacion{queTipoHay}{pos, $\langle$ cAs${,}$ \ cH${,}$ \ cEst $\rangle$}{tipo}{exista c en claves que cumpla que: (si está en claves de dAs entonces obtener de c en dAs da pos y si está en claves de dH entonces etc.) }
...
\tadAxioma{campus? (moverHoE($u, i$))}{campus?($u$)\\}
