...
\tadOperacion{agregarE}{uni, id, ($\langle est$, $pos \rangle$)}{uni}{}
...
\tadAxioma{agentes?(nuevaUni(al,an,co,cAs))}{cAs}
\tadAxioma{agentes?(agregarH(uni,hip,pos))}{\IF movRest $\in$ queSituacion(cuatroVecinos(pos, uni)) THEN 
		capturar(pos, agentes?(uni)) ELSE agentes?(uni)	FI}
\tadAxioma{agentes?(agregarE(uni,id,$\langle est$, $pos \rangle$))}{\IF movRest $\in$ queSituacion(cuatroVecinos(pos, uni)) THEN 
	sancionar(pos, agentes?(uni)) ELSE agentes?(uni)	FI}
\tadAxioma{agentes?(moverAs(uni,i))}{prim(moverAgenteYSancionarYCapturar(i, uni))}
\tadAxioma{agentes?(moverE(uni,i))}{prim(moverEstudiante(i,uni))}
\tadAxioma{agentes?(moverH(uni,i))}{prim(moverHippie(i,uni))}

\tadAxioma{hippies?(nuevaUni(al,an,co,cAs))}{$\emptyset$}

\tadAxioma{hippies?(agregarH(uni,hip, pos))}{
